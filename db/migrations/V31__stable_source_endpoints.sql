...
ALTER TABLE topic_sources
    ...
    ADD CONSTRAINT ck_topic_sources_status CHECK (status IN ('active', 'inactive')),
    ADD CONSTRAINT ck_topic_sources_inactive_at CHECK (
...
CREATE TABLE source_endpoints (
    ...
    CONSTRAINT ck_source_endpoints_inactive_at CHECK (
        (status = 'active' AND inactive_at IS NULL)
        OR (status = 'inactive' AND inactive_at IS NOT NULL))
);
