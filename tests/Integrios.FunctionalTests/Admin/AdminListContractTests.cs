...
public sealed class AdminListContractTests(AdminApiFixture fixture) : AdminApiTestBase, IClassFixture<AdminApiFixture>, IAsyncLifetime
{
    ...
    public async Task Lists_FilterByTheirApprovedFieldsAndOmitDetailConfiguration()
    {
        ...
        await ExecuteAsync($$$"""
            UPDATE tenants SET status = 'inactive' WHERE id = @OtherTenantId;
            UPDATE destinations SET status = 'inactive' WHERE id = @DestinationId;
            INSERT INTO tenant_api_keys (id, tenant_id, name, key_prefix, key_hash, created_at, revoked_at)
            VALUES (@RevokedKeyId, @TenantId, 'revoked-list-key', 'ik_revoked', 'sha256:test', @Now, @Now);
            ...
            VALUES (@SourceId, @TenantId, @SourceConnectorId, @TopicId, 'disabled-list-intake', 'event_api', '["list.contract"]', {{{fixture.Json("@Configuration")}}}, 'fixture-revision', 'inactive', @Now, @Now);
            INSERT INTO subscriptions (id, tenant_id, topic_id, name, event_types, destination_id, order_index, status, created_at, updated_at)
            VALUES (@SubscriptionId, @TenantId, @TopicId, 'disabled-list-subscription', {{{fixture.Json("@EventTypes")}}}, @DestinationId, 0, 'inactive', @Now, @Now);
            """,
            new
            {
        ...
        (await ListIdsAsync("/admin/tenants?status=inactive")).ShouldContain(fixture.OtherTenantId);
        ...
        (await ListIdsAsync($"/admin/tenants/{fixture.TenantId}/sources?status=inactive&type=event_api")).ShouldContain(sourceId);
        (await GetListAsync($"/admin/tenants/{fixture.TenantId}/sources?status=inactive&type=event_api")).GetProperty("items")[0].GetProperty("type").GetString().ShouldBe("event_api");
        (await ListIdsAsync($"/admin/tenants/{fixture.TenantId}/topics/{topicId}/subscriptions?status=inactive")).ShouldContain(subscriptionId);
    ...
    public async Task SubscriptionsByTenant_FilterAndNameTheirTopicAndDestination()
    {
        ...
        Guid firstDestination = Guid.NewGuid();
        Guid secondDestination = Guid.NewGuid();
        ...
        await ExecuteAsync($$$"""
            ...
            (@Second, @TenantId, @FirstTopic, 'Archive orders', {{{fixture.Json("@Rules")}}}, @SecondDestination, 2, 'inactive', @Now, @Now),
        ...
        row.GetProperty("destination_name").GetString().ShouldBe("Primary CRM");
        ...
        topicRow.GetProperty("destination_name").GetString().ShouldBe("Primary CRM");
        topicRow.TryGetProperty("topic_name", out _).ShouldBeFalse();
        (await ListIdsAsync($"{root}?topic_id={firstTopic}&destination_id={secondDestination}&status=inactive")).ShouldBe([second]);
    ...
    public async Task Tenants_SearchNameOrSlugAndEnvironment_AndBindEachFilterToCursor()
    {
        ...
        await ExecuteAsync("""
            ...
            (@Other, 'other-search', 'Needle elsewhere', 'staging', 'inactive', @Now, @Now)
    ...
    [Fact]
    public async Task Destinations_MatchEnvironmentWithoutCasingAndKeepFreeTextOutOfTheCursorScope()
    {
        Guid first = Guid.NewGuid();
        Guid second = Guid.NewGuid();
        await ExecuteAsync($$"""
    ...
    public async Task Sources_ProjectContract_FilterTopic_AndKeepCursorTenantAndFilterScope()
    {
        ...
        await ExecuteAsync($$"""
            ...
            (@First, @TenantId, @ConnectorId, @Topic, 'first-intake', 'event_api', '["list.contract"]', {{fixture.Json("@Configuration")}}, 'fixture-revision', 'active', @Now, @Now),
            (@Second, @TenantId, @ConnectorId, @Topic, 'second-intake', 'webhook', '["list.contract"]', {{fixture.Json("@Configuration")}}, 'fixture-revision', 'inactive', @Now, @Now),
            (@Excluded, @TenantId, @ConnectorId, @OtherTopic, 'excluded-intake', 'broker', '["list.contract"]', {{fixture.Json("@Configuration")}}, 'fixture-revision', 'active', @Now, @Now)
            """, new
        {
            Topic = topic,
