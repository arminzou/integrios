    ...
    : IClassFixture<BrokerSourceReconciliationFixture>, IAsyncLifetime
{
    ...
    public async Task SourceRevokedAfterStartup_StopsConsuming()
    {
        ...
        (await fixture.WaitForEventAsync(ignored, NotConsumed)).ShouldBeNull();
    ...
    public async Task SourceConfigurationChanged_RebindsTheProcessor()
    {
        ...
        (await fixture.WaitForEventAsync(whileRebound, NotConsumed)).ShouldBeNull();

        await fixture.SetQueueNameAsync(sourceId, AzureServiceBusSourceTests.QueueName);
...
public sealed class BrokerSourceReconciliationFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        Seeded = await AzureServiceBusSourceTests.SeedAsync(Database, includeSource: false);
    ...
    internal Task RevokeSourceAsync(Guid sourceId) => ExecuteAsync(
        $"UPDATE sources SET status = 'inactive' WHERE id = @Id",
        new { Id = sourceId });
    ...
    internal Task SetQueueNameAsync(Guid sourceId, string queueName)
    {
        string configuration = JsonSerializer.Serialize(new
        {
            ...
            authentication = new
            {
                ...
                secret_ref = AzureServiceBusSourceTests.SecretReference,
            },
    ...
    internal async Task PublishAsync(object body)
    {
        ...
        ServiceBusSender sender = client.CreateSender(AzureServiceBusSourceTests.QueueName);
    ...
    internal async Task DrainQueueAsync()
    {
        ...
        await using ServiceBusReceiver receiver = client.CreateReceiver(
            AzureServiceBusSourceTests.QueueName);
