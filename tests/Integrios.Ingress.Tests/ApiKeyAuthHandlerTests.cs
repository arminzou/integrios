    ...
    : IClassFixture<ApiTestAppFixture>, IAsyncLifetime
{
    ...
    {
        HttpResponseMessage response = await PostEventsAsync($"ApiKey {TestToken}");
        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }
    ...
    {
        (ApiKey apiKey, Tenant tenant) = BuildValidApiKey(TestToken);
        fixture.ApiKeyRepository.Result = (apiKey, tenant);

        HttpResponseMessage response = await PostEventsAsync($"ApiKey {WrongToken}");
        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }
    ...
    public static (ApiKey ApiKey, Tenant Tenant) BuildValidApiKey(string token)
    {
