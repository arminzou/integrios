...
public sealed class AzureServiceBusSourceIntegrationTests(AzureServiceBusSourceFixture fixture)
    : IClassFixture<AzureServiceBusSourceFixture>
{
    ...
    public async Task TopicSubscriptionMessage_CompletesAndCreatesEvent()
    {
        ...
        Guid attributedSource = await QuerySingleAsync<Guid>(
            ...
            new { Id = eventId });
        Assert.Equal(fixture.TopicSourceId, attributedSource);
    }
...
public sealed class AzureServiceBusSourceFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        SeededQueueSource seeded = await AzureServiceBusSourceIntegrationTests.SeedAsync(Database);
        TenantId = seeded.TenantId;
        await AzureServiceBusSourceIntegrationTests.InsertQueueSourceAsync(
            Database, seeded, TopicSourceId,
            AzureServiceBusSourceIntegrationTests.TopicSourceConfigurationJson());
