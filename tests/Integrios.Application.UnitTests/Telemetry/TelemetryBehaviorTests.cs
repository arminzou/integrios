...
public sealed class TelemetryBehaviorTests
{
    ...
    public async Task Handle_StartsRootAcceptanceSpanForEveryIngressCommand()
    {
        ...
        await HandleAcceptanceAsync(new AcceptBrokerMessageCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null, null, JsonDocument.Parse("{}").RootElement, null, null), accepted);
