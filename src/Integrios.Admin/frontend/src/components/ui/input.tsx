...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      type={type}
      data-slot="input"
      // Authoring forms here describe a deployment, never the Operator's own details, so a value
      // the browser has saved is never the right suggestion. A field that genuinely wants one
      // passes its own, because this sits ahead of the caller's props.
      autoComplete="off"
      className={cn(
