...
describe("Editing a Subscription", () => {
  ...
  it("offers Activate and editing once it is inactive", async () => {
    const calls = stubEdit({ ...subscription, status: "inactive" });
    renderScreen(
      <SubscriptionsScreen tenantId={tenantId} selectedTopicId={topicId} selectedSubscriptionId={subscriptionId} />,
      detailPath,
    );
    ...
    await waitFor(() =>
      expect(
        calls.some((call) => call.method === "POST" && call.url.pathname.endsWith(`/${subscriptionId}/activate`)),
      ).toBe(true),
    );
  });
});
