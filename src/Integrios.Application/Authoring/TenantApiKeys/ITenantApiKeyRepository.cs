...
public interface ITenantApiKeyRepository
{
    ...
    Task<(IReadOnlyList<TenantApiKey> Items, string? NextCursor)> ListByTenantAsync(Guid tenantId, TenantApiKeyListState? state, DateTimeOffset now, string? afterCursor, int limit, CancellationToken cancellationToken);
