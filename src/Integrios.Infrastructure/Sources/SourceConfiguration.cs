...
internal sealed class SourceConfiguration : IEntityTypeConfiguration<Source>
{
    public void Configure(EntityTypeBuilder<Source> entity)
    {
        ...
        entity.HasIndex(source => source.ConnectorId, "idx_sources_connector_id");
        ...
        entity.HasOne<Connector>().WithMany().HasForeignKey(source => source.ConnectorId)
            .OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("fk_sources_connector");
