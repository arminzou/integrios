    ...
    public async Task<SourceListDto> ListAsync(Guid tenantId, SourceStatus? status, SourceType? type, Guid? topicId, string? afterCursor, int limit, CancellationToken cancellationToken)
    {
        ...
        string sql = $"""
            SELECT {(sqlServer ? "TOP (@Take)" : "")}
                id AS Id, tenant_id AS TenantId, connection_id AS ConnectionId, topic_id AS TopicId,
                type AS Type, status AS Status,
                {(sqlServer
                    ? "COALESCE(JSON_VALUE(configuration, '$.source_contract'), '')"
                    : "COALESCE(configuration ->> 'source_contract', '')")} AS SourceContract,
        ...
        return new SourceListDto(rows.Select(row => new SourceListItemDto(
            row.Id, row.TenantId, row.ConnectionId, row.TopicId, row.Type, row.Status, row.SourceContract,
