...
internal sealed class ConnectorConfiguration : IEntityTypeConfiguration<Connector>
{
    ...
    {
        entity.HasKey(e => e.Id).HasName("connectors_pkey");

        entity.ToTable("connectors", table =>
        {
            table.HasCheckConstraint("ck_connectors_contract_version_positive", "contract_version > 0");
            table.HasCheckConstraint(
                "ck_connectors_manifest_schema_version_positive",
                "manifest_schema_version > 0");
            table.HasCheckConstraint("ck_connectors_manifest_object", "jsonb_typeof(manifest) = 'object'");
            table.HasCheckConstraint(
